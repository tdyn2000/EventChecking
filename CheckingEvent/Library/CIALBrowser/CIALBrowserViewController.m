...
{
    [self shareText:_urlToLoad.absoluteString andImage:nil andUrl:nil];
}
...
- (void)reloadView {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        if (self.isModal) {
            ...
            NSString *shareTitle = CIALBrowserLocalizedString(@"Share", nil);
            closeButtonItem = [[UIBarButtonItem alloc] initWithTitle:shareTitle style:UIBarButtonItemStyleBordered target:self action:@selector(share:)];
            navigationItem.rightBarButtonItem = closeButtonItem;
            [buttons addObject:closeButtonItem];
//            [buttons addObject:flexibleSpaceButtonItem];
        }
